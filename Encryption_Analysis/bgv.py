...
def poly_mod(x, mod, n):
    ...
    for i in range(n, len(x)):
        x[i - n] -= x[i]  # because x^n ≡ -1 mod (x^n+1)
    return np.remainder(x, mod)

def generate_keys(n, q, t, sigma, seed=None):
    # For this toy simulation, BGV key generation is similar to BFV.
    if seed is not None:
        np.random.seed(seed)
    ...
    pk1 = a.copy()
    return {"n": n, "q": q, "t": t, "sigma": sigma, "sk": sk, "pk": (pk0, pk1), "seed": seed}
...
def encrypt(params, m):
    ...
    # Encrypt similarly as BFV (a modulus switching step is conceptually done during decryption)
